import { getAccountsOAuthBaseUrl, getBearerOrCookieAccessToken } from '../../utils/accounts';
...
export default defineEventHandler(async (event) => {
  ...
  if (accessToken) {
    try {
      await $fetch(`${getAccountsOAuthBaseUrl(event)}/api/auth/sessions/revoke-others`, {
        method: 'POST',
        headers: {
