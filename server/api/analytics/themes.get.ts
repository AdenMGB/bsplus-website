...
export default defineEventHandler(async (event) => {
  ...
  try {
    ...
    const topByDownloads = await db
      .prepare(
        `SELECT id, name, author, download_count, favorite_count, rating_average, rating_count
         FROM themes
         WHERE status = 'approved'
    ...
    return {
      ...
      byCategory: (byCategory?.results || []).map((r: any) => ({
        category: r.category,
        count: r.count,
        downloads: r.downloads,
      })),
      topByDownloads: (topByDownloads?.results || []).map((r: any) => ({
        id: r.id,
        name: r.name,
        author: r.author,
