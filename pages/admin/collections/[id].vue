  ...
  <div class="py-24 sm:py-32">
    <div class="mx-auto max-w-4xl px-6 lg:px-8">
      <div class="flex items-center justify-between mb-16">
        <div>
          <h2 class="text-3xl font-bold tracking-tight text-white sm:text-4xl">Edit Collection</h2>
          <p class="mt-2 text-lg text-zinc-400">Update collection details</p>
        </div>
      ...
      <div v-if="collection" class="space-y-8">
        ...
        <div class="bg-zinc-900/50 border border-zinc-800 rounded-2xl p-8">
          ...
          <div class="flex items-center gap-4">
            <div v-if="coverImagePreview || collection.cover_image_url" class="relative group h-32 w-48 rounded-lg overflow-hidden border border-zinc-700 bg-zinc-900">
              <img :src="coverImagePreview || collection.cover_image_url" alt="Cover" class="h-full w-full object-cover" />
...
const { data: collectionData } = await useFetch<any>(`/api/collections/${collectionId}`);
const collection = computed(() => collectionData.value?.data?.collection);
...
watch(collection, (col) => {
  if (col) {
    form.value = {
      ...
      cover_image: col.cover_image_url || ''
    };
    coverImagePreview.value = col.cover_image_url || '';
    // Load selected themes
    if (col.themes) {
      selectedThemes.value = col.themes;
    }
  }
}, { immediate: true });
  ...
  try {
    await $fetch(`/api/admin/collections/${collectionId}`, {
      ...
      body: {
        ...
        cover_image: form.value.cover_image || collection.value?.cover_image_url
      }
    });
    router.push('/admin/collections');
  } catch (e: any) {
    alert('Failed to update collection: ' + (e.data?.error?.message || e.message || 'Unknown error'));
  } finally {
    saving.value = false;
  }
}
