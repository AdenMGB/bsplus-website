    ...
    <div class="mx-auto max-w-7xl px-6 lg:px-8">
      <div class="flex items-center justify-between mb-16">
        ...
        <div class="flex gap-4">
          <NuxtLink to="/admin/news/create" class="flex items-center gap-2 rounded-md bg-green-600 px-4 py-2.5 text-sm font-semibold text-white shadow-sm hover:bg-green-500 transition-all hover:scale-105">
      ...
      <div class="grid grid-cols-1 gap-8 md:grid-cols-3 mb-16">
        ...
        <div class="bg-zinc-900/50 border border-zinc-800 rounded-2xl p-6">
          ...
          <dd class="mt-2 text-3xl font-bold tracking-tight text-white">{{ stats.totalPosts || 0 }}</dd>
        </div>
        <div class="bg-zinc-900/50 border border-zinc-800 rounded-2xl p-6">
          <dt class="text-sm font-medium leading-6 text-zinc-400">Published</dt>
          <dd class="mt-2 text-3xl font-bold tracking-tight text-white">{{ stats.publishedPosts || 0 }}</dd>
        </div>
        <div class="bg-zinc-900/50 border border-zinc-800 rounded-2xl p-6">
          <dt class="text-sm font-medium leading-6 text-zinc-400">Total Views</dt>
          <dd class="mt-2 text-3xl font-bold tracking-tight text-white">-</dd>
        </div>
      </div>
...
const stats = computed(() => {
  if (!posts.value) return { totalPosts: 0, publishedPosts: 0 };
  return {
    totalPosts: posts.value.length,
    publishedPosts: posts.value.filter(p => p.published).length
  };
});
