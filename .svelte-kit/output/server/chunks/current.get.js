...
//#region server/api/questionnaire/current.get.ts
    ...
    `).bind(now).first();
      ...
      `).first();
		...
		if (question.expires_at <= now) {
			await db.prepare("UPDATE daily_questions SET is_active = 0 WHERE id = ?").bind(question.id).run();
			const queuedQuestion = await db.prepare(`
        SELECT * FROM daily_questions 
        WHERE auto_activate = 1 AND is_active = 0 AND duration IS NOT NULL
        ORDER BY queue_order ASC, created_at ASC
...
//#endregion
export { current_get_default as default };
