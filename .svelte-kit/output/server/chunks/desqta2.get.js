//#region server/api/changelogs/desqta.get.ts
var desqta_get_default = defineEventHandler(async (event) => {
	const query = getQuery(event);
	const page = Number(query.page) || 1;
	const perPage = Number(query.per_page) || 10;
	try {
		const response = await fetch(`https://api.github.com/repos/betterseqta/desqta/releases?page=${page}&per_page=${perPage}`, { headers: {
