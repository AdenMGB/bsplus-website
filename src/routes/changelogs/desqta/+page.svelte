...
<section class="mx-auto max-w-5xl space-y-8 pb-14 pt-6">
  ...
  <div class="space-y-3">
    <div class="section-subtitle">DesQTA changelog</div>
    <h1 class="section-title">Complete release history for the desktop app.</h1>
  </div>
    ...
    {#each releases as release (release.id)}
      <Card className="overflow-hidden p-0">
        <button class="flex w-full items-center justify-between gap-4 px-6 py-5 text-left" type="button" onclick={() => toggleRelease(release.id)}>
          <div>
            <div class="text-xl font-semibold text-white">{release.name || release.tag_name}</div>
            <div class="mt-2 text-sm text-soft">
              {release.tag_name} · {formatDate(release.published_at)} · {release.author}
            </div>
          </div>
